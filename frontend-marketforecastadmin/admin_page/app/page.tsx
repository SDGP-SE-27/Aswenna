...
const ActionModal: React.FC<ActionModalProps> = ({ entry, onClose, onEdit, onDelete }) => {
  return (
    <div className="fixed inset-0 flex items-center justify-center z-50" onClick={onClose}>
      <div className="bg-white/90 backdrop-blur-sm rounded-lg shadow-xl w-64 border border-gray-200" onClick={e => e.stopPropagation()}>
        {/* Header */}
...
export default function Home() {
  ...
  const [deleteConfirmEntry, setDeleteConfirmEntry] = useState<PriceEntry | null>(null);
  ...
  const fetchPrices = async () => {
    ...
    try {
      ...
      setPrices(response.data.prices?.length > 0 ? response.data.prices : generateDummyData());
    } catch (err) {
  ...
  const handleSave = () => {
    ...
    if (isNewEntry) {
      setPrices(prev => [...prev, editingEntry].sort((a, b) => a.date.localeCompare(b.date)));
    } else {
      setPrices(prev => 
        prev.map(entry => 
  ...
  const handleDeleteConfirm = () => {
    ...
    setPrices(prev => prev.filter(entry => entry.date !== deleteConfirmEntry.date));
    ...
    <div className="relative">
      ...
      >
        <div className="w-6 h-6 flex flex-col justify-between">
          <span className={`block h-0.5 w-full bg-current transform transition duration-300 ${isMenuOpen ? 'rotate-45 translate-y-2.5' : ''}`} />
          <span className={`block h-0.5 w-full bg-current transition duration-300 ${isMenuOpen ? 'opacity-0' : ''}`} />
          <span className={`block h-0.5 w-full bg-current transform transition duration-300 ${isMenuOpen ? '-rotate-45 -translate-y-2.5' : ''}`} />
        </div>
      </button>
        ...
        <div>
          <ul className="list-none">
            {CROP_TYPES.map((crop) => (
              <li 
                key={crop} 
                className={`p-4 hover:bg-gray-800 cursor-pointer ${cropType === crop ? 'bg-gray-800' : ''}`}
          ...
          <div className="bg-white rounded-lg p-6 w-full max-w-md">
            ...
            <div className="space-y-4">
              ...
              <div>
                ...
                <input
                  ...
                  onChange={(e) =>
                    setEditingEntry({
                      ...editingEntry,
                      retail_price: e.target.value === '' ? null : parseFloat(e.target.value),
                    })
                  }
                  className="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm"
                />
              ...
              <div>
                ...
                <input
                  ...
                  onChange={(e) =>
                    setEditingEntry({
                      ...editingEntry,
                      ncpi_food: e.target.value === '' ? null : parseFloat(e.target.value),
                    })
                  }
                  className="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm"
                />
              ...
              <div>
                ...
                <input
                  ...
                  onChange={(e) =>
                    setEditingEntry({
                      ...editingEntry,
                      ncpi_non_food: e.target.value === '' ? null : parseFloat(e.target.value),
                    })
                  }
                  className="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm"
                />
              ...
              <div>
                ...
                <input
                  ...
                  onChange={(e) =>
                    setEditingEntry({
                      ...editingEntry,
                      ncpi_all_items: e.target.value === '' ? null : parseFloat(e.target.value),
                    })
                  }
                  className="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm"
                />
              ...
              <div>
                ...
                />
                <p className="mt-1 text-sm text-gray-500">Predicted price cannot be modified</p>
              </div>
            </div>
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-white rounded-lg p-6 w-full max-w-md">
            ...
            <p className="text-gray-600 mb-6">
              Are you sure you want to delete the entry for {deleteConfirmEntry.date}? This action cannot be undone.
            </p>
